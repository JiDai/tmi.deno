...
const msg_channel_suspended =
	'@msg-id=msg_channel_suspended :tmi.twitch.tv NOTICE #local7000 :This channel has been suspended.';
...
const tests = [
	...
	},
	{
		...
		returnedParams: ['#local7000'],
		serverTest: 'PART',
		serverCommand(client: Client, ws: WebSocket) {
	...
	},
	{
		...
		errorCommands: ['@msg-id=usage_mods :tmi.twitch.tv NOTICE #local7000 :Usage: "/mods"'],
	},
	{
		command: 'mods',
		inputParams: ['#local7000'],
		...
		serverTest: '/mods',
		serverCommand:
			'@msg-id=no_mods :tmi.twitch.tv NOTICE #local7000 :There are no moderators of this channel.',
	},
	{
		command: 'part',
		inputParams: ['#local7000'],
	...
	},
	{
		...
		serverTest: '/vip',
		serverCommand:
			'@msg-id=vip_success :tmi.twitch.tv NOTICE #local7000 :You have added User as a vip of this channel.',
		errorCommands: [
	...
	},
	{
		...
		serverTest: '/unvip',
		serverCommand:
			'@msg-id=unvip_success :tmi.twitch.tv NOTICE #local7000 :You have removed User as a VIP of this channel..',
		errorCommands: [
	...
	},
	{
		...
		serverTest: '/vips',
		serverCommand:
			'@msg-id=no_vips :tmi.twitch.tv NOTICE #local7000 :This channel does not have any VIPs.',
	},
	{
		command: 'whisper',
		...
		errorCommands: [no_permission],
	},
];

Rhum.testPlan('My Plan', () => {
	Rhum.testSuite('commands (justinfan)', () => {
		let server: WebSocketServer;
		let client: Client;

		Rhum.beforeEach(function () {
			// Initialize websocket server
			server = new WebSocketServer(4444);
			client = new Client({
				connection: {
					server: 'localhost',
					port: 4444,
		...
		Rhum.testCase('handles commands when disconnected', function () {
			client.subscribers('local7000').then(noop, (err) => {
				Rhum.asserts.assertEquals(err, 'Not connected to server.');
			});
		});

		Rhum.testCase('handles ping', function () {
		...
		Rhum.testCase('handles ping timeout', function () {
			server.on('connection', (ws) => {
		...
		tests.forEach((test) => {
			Rhum.testCase(`should handle ${test.command}`, function () {
				server.on('connection', (ws) => {
					ws.on('message', (message: string) => {
				...
				client.addListener('join', function () {
					...
					client[test.command].apply(this, test.inputParams).then((data) => {
						test.returnedParams.forEach((param, index) => {
							Rhum.asserts.assertEquals(data[index], param);
						});
						client.disconnect();
					});
				});
			...
			if (test.errorCommands) {
				test.errorCommands.forEach((error) => {
					Rhum.testCase(`should handle ${test.command} errors`, function () {
						server.on('connection', (ws) => {
							ws.on('message', (message: string) => {
								// Ensure that the message starts with NICK
								if (!message.indexOf('NICK')) {
			...
			if (test.testTimeout) {
				Rhum.testCase(`should handle ${test.command} timeout`, function () {
					server.on('connection', (ws) => {
						ws.on('message', (message: string) => {
							// Ensure that the message starts with NICK
							if (!message.indexOf('NICK')) {
	...
	Rhum.testSuite('commands (identity)', () => {
		let server: WebSocketServer;
		let client: Client;

		Rhum.beforeEach(function () {
			// Initialize websocket server
			server = new WebSocketServer(7000);
			client = new Client({
				connection: {
					server: 'localhost',
					port: 7000,
		...
		Rhum.testCase('should handle action', function () {
			server.on('connection', (ws) => {
				ws.on('message', (message: string) => {
			...
			client.addListener('logon', () => {
				client.action('#local7000', 'Hello').then((dataRaw: unknown) => {
					const data = dataRaw as Array<string>;
					Rhum.asserts.assertEquals(data[0], '#local7000');
					Rhum.asserts.assertEquals(data[1], '\u0001ACTION Hello\u0001');
					client.disconnect();
				});
			});
		...
		Rhum.testCase('should handle say', function () {
			server.on('connection', (ws) => {
				ws.on('message', (message: string) => {
					if (~message.indexOf('Hello')) {
			...
			client.addListener('logon', () => {
				client.say('#local7000', 'Hello').then((dataRaw: unknown) => {
					const data = dataRaw as Array<string>;
					Rhum.asserts.assertEquals(data[0], '#local7000');
					Rhum.asserts.assertEquals(data[1], 'Hello');
					client.disconnect();
				});
			});
		...
		Rhum.testCase('should handle say when disconnected', function () {
			client.say('#local7000', 'Hello!').then(noop, (err) => {
				Rhum.asserts.assertEquals(err, 'Not connected to server.');
			});
		});

		Rhum.testCase('should break up long messages (> 500 characters)', function () {
			...
			client.addListener('chat', (channel: string, user: string, message: string) => {
				calls++;
				if (calls > 1) {
					Rhum.asserts.assertStringIncludes(message, 'ipsum');
					client.disconnect();
				}
			});
		...
		Rhum.testCase('should break up long messages without spaces (> 500 characters)', function () {
		...
		['/me', '\\me', '.me'].forEach((me) => {
			Rhum.testCase(`should handle ${me} say`, function () {
				server.on('connection', (ws) => {
					ws.on('message', (message: string) => {
						if (~message.indexOf('Hello')) {
							ws.send(':tmi.twitch.tv PRIVMSG #local7000 :Hello');
				...
				client.addListener('logon', () => {
					client.say('#local7000', `${me} Hello`).then((dataRaw: unknown) => {
						const data = dataRaw as Array<string>;
						Rhum.asserts.assertEquals(data[0], '#local7000');
						Rhum.asserts.assertEquals(data[1], '\u0001ACTION Hello\u0001');
						client.disconnect();
		...
		['.', '/', '\\'].forEach((prefix) => {
			Rhum.testCase(`should handle ${prefix} say`, function () {
				client.addListener('logon', () => {
					client.say('#local7000', `${prefix}FOO`).then((dataRaw: unknown) => {
						const data = dataRaw as Array<string>;
						Rhum.asserts.assertEquals(data[0], '#local7000');
						Rhum.asserts.assertEquals(data.length, 2);
						client.disconnect();
					});
				});
		...
		['..'].forEach((prefix) => {
			Rhum.testCase(`should handle ${prefix}message say`, function () {
				client.addListener('logon', () => {
					client.say('#local7000', `${prefix}FOO`).then((dataRaw: unknown) => {
						const data = dataRaw as Array<string>;
						Rhum.asserts.assertEquals(data[0], '#local7000');
						Rhum.asserts.assertEquals(data[1], `${prefix}FOO`);
						Rhum.asserts.assertEquals(data.length, 2);
						client.disconnect();
					});
				});
